...
RULES: tuple[IntentRule, ...] = (
    ...
    IntentRule(
        "virus_scan",
        ("فيروس", "فيروسات", "مصاب", "برامج خبيثه",
         "malware", "virus", "infected", "trojan"),
        (ArgSpec(FILE, "pick_file_scan"),),
        priority=10,
    ),
    IntentRule(
        "security_report",
        ("افحص .*امني", "فحص امني", "تقرير امني", "امان الملف",
         "الملف ده امان", "افحص الملف ده",
         "security report", "security scan", "is .* safe", "check .* safe"),
        (ArgSpec(FILE, "pick_file_scan"),),
        priority=9,
    ),
    IntentRule(
        ...
        ("افحص الكود", "فحص كود", "ثغرات في الكود", "مشاكل امنيه في الكود",
         "راجع الكود", "code scan", "review .*code", "scan .*code", "lint"),
        (ArgSpec(FILE, "pick_code"),),
    ),
    IntentRule(
        "vuln_scan",
        ("ثغرات", "اسرار مكشوفه", "مفاتيح مكشوفه",
         "vulnerabilit", "exposed secret", "leaked key", "cve"),
        (ArgSpec(DIR, "pick_dir"),),
    ),
    IntentRule("quarantine_list", ("الحجر الصحي", "الملفات المعزوله", "quarantine")),
    IntentRule(
        "hash_file",
        ("بصمه الملف", "هاش الملف", "تجزئه الملف",
         "checksum", "hash .*file", "sha256", "md5"),
        (ArgSpec(FILE, "pick_file"),),
    ),
    IntentRule(
        "tls_check",
        ("شهاده", "ssl", "tls", "https امان", "certificate", "cert expir"),
        (ArgSpec(HANDLE, "pick_host"),),
    ),
    IntentRule(
        "file_perms",
        ("صلاحيات", "الصلاحيات", "permission", "file mode", "chmod"),
        (ArgSpec(FILE, "pick_file"),),
    ),
    ...
    IntentRule(
        "extract_audio",
        ("استخرج الصوت", "طلع الصوت", "الصوت بس", "حول .* mp3",
         "extract audio", "rip audio", "audio only", "to mp3"),
        (ArgSpec(FILE, "pick_video", filetypes="video"),
         ArgSpec(OUT, suffix="_audio", ext=".mp3")),
        priority=8,
    ),
    IntentRule(
        "convert",
        ("حول الفيديو", "حول الملف", "غير صيغه",
    ...
    IntentRule(
        "auto_trim_silence",
        ("شيل الصمت", "قص الصمت", "نضف الصمت", "الاجزاء الميته",
         "trim silence", "remove silence", "cut .*silence", "dead air"),
        (ArgSpec(FILE, "pick_media", filetypes="media"),
         ArgSpec(OUT, suffix="_trimmed")),
        priority=8,
    ),
    IntentRule(
        "color_grade",
        ("تدرج لوني", "تصحيح الوان", "لوك سينمائي", "الوان سينمائيه",
         "color grade", "colour grade", "cinematic look", "color correct"),
        (ArgSpec(FILE, "pick_video", filetypes="video"),
         ArgSpec(OUT, suffix="_graded")),
    ),
    IntentRule(
        "stabilize",
        ("ثبت الفيديو", "الاهتزاز", "شيل الرجفه",
         "stabili[sz]e", "shaky", "smooth .*footage"),
        (ArgSpec(FILE, "pick_video", filetypes="video"),
         ArgSpec(OUT, suffix="_stable")),
    ),
    IntentRule(
        "speed_ramp",
        ("سرع الفيديو", "بطء الفيديو", "سلو موشن", "غير السرعه",
         "speed up", "slow motion", "slow down", "change speed"),
        (ArgSpec(FILE, "pick_video", filetypes="video"),
    ...
    IntentRule(
        "denoise_audio",
        ("شيل الضوضاء", "نضف الصوت", "ضوضاء",
         "denoise", "remove noise", "clean .*audio", "background noise"),
        (ArgSpec(FILE, "pick_audio", filetypes="audio"),
         ArgSpec(OUT, suffix="_clean")),
    ),
    IntentRule(
        "master_audio",
        ("اضبط الصوت", "مستوي الصوت", "جهاره",
         "master audio", "loudness", "normali[sz]e audio", "lufs"),
        (ArgSpec(FILE, "pick_audio", filetypes="audio"),
         ArgSpec(OUT, suffix="_mastered")),
    ),
    IntentRule(
        "detect_scenes",
        ("المشاهد", "قسم الفيديو", "تغيير المشهد",
         "detect scene", "split .*scene", "scene change", "shot detect"),
        (ArgSpec(FILE, "pick_video", filetypes="video"),),
    ),
    IntentRule(
        "separate_vocals",
        ("افصل الصوت", "افصل الغنا", "اعزل الصوت", "الموسيقي لوحدها",
         "separate vocal", "isolate vocal", "split stem", "remove vocal",
         "instrumental"),
        (ArgSpec(FILE, "pick_audio", filetypes="audio"),
         ArgSpec(DIR, "pick_out_dir")),
    ),
    IntentRule(
        "upscale_video",
        ("كبر الفيديو", "جوده الفيديو", "دقه اعلي للفيديو",
         "upscale .*video", "enhance .*video", "4k .*video"),
        (ArgSpec(FILE, "pick_video", filetypes="video"),
    ...
    IntentRule(
        "upscale_image",
        ("كبر الصوره", "جوده الصوره", "دقه اعلي",
         "upscale .*image", "enhance .*(image|photo)", "sharpen"),
        (ArgSpec(FILE, "pick_image", filetypes="image"),
         ArgSpec(OUT, suffix="_big")),
    ),

    # ── صوت نيزوكو ────────────────────────────────────────────────
    IntentRule("speak", ("قولي", "اقري", "انطقي", "قول ",
     "say ", "speak ", "read out", "read this"), (ArgSpec(REST),), priority=5),
    IntentRule("listen", ("اسمعيني", "سجل صوت", "شغل المايك",
     "listen", "record .*mic", "transcribe me", "voice input")),
    IntentRule("voice_status", ("حاله الصوت", "الصوت شغال", "voice status", "tts status")),

    # ── يوتيوب ────────────────────────────────────────────────────
    IntentRule(
        "channel_stats",
        ("احصائيات قناه", "بيانات قناه", "معلومات قناه",
         "channel stat", "channel info", "subscriber count"),
        (ArgSpec(HANDLE, "pick_channel"),),
        priority=8,
    ),
    IntentRule(
        "channel_growth_report",
        ("نمو القناه", "تحليل القناه", "القناه بتكبر", "اداء القناه",
         "channel growth", "growth report", "why .*not growing",
         "channel performance"),
        (ArgSpec(HANDLE, "pick_channel"),),
        priority=9,
    ),
    IntentRule(
        "video_stats",
        ("احصائيات الفيديو", "مشاهدات الفيديو",
         "video stat", "view count", "how many views"),
        (ArgSpec(HANDLE, "pick_video_id"),),
    ),
    IntentRule(
        "youtube_search",
        ("دور في يوتيوب", "ابحث في يوتيوب",
         "youtube search", "search youtube", "find .*on youtube"),
        (ArgSpec(REST),),
    ),
    IntentRule("trending_videos", ("الترند", "الرايج", "trending", "what.*popular")),
    IntentRule(
        "seo_title_score",
        ("قيم العنوان", "العنوان ده كويس", "seo title", "score .*title",
         "rate .*title"),
        (ArgSpec(REST),),
    ),
    IntentRule(
        "hook_analyzer",
        ("قيم الهوك", "البدايه دي", "hook", "opening line", "intro strength"),
        (ArgSpec(REST),),
    ),
    IntentRule(
        "thumbnail_analyze",
        ("حلل المصغره", "المصغره دي",
         "thumbnail", "analy[sz]e .*thumb"),
        (ArgSpec(FILE, "pick_image", filetypes="image"),),
    ),
    IntentRule(
        "comment_sentiment",
        ("مشاعر التعليقات", "التعليقات ايجابيه", "راي الناس",
         "sentiment", "comment mood", "what .*people say"),
        (ArgSpec(REST),),
    ),
    ...
    IntentRule(
        "csv_describe",
        ("حلل الملف ده csv", "احصائيات csv", "وصف البيانات",
         "describe .*csv", "csv stat", "summari[sz]e .*(csv|data)"),
        (ArgSpec(FILE, "pick_csv"),),
    ),
    IntentRule(
        "db_schema",
        ("جداول قاعده البيانات", "بنيه قاعده",
         "schema", "database table", "show .*tables"),
        (ArgSpec(FILE, "pick_db"),),
    ),

    # ── أندرويد ───────────────────────────────────────────────────
    IntentRule("android_devices", ("الاجهزه المتصله", "الموبايلات",
         "android device", "connected phone", "adb device")),
    IntentRule(
        "android_install",
        ("ثبت التطبيق", "نزل apk", "install apk", "sideload"),
        (ArgSpec(FILE, "pick_apk"),),
    ),
    IntentRule(
        "android_screenshot",
        ("صوره من الموبايل", "سكرين شوت للموبايل",
         "android screenshot", "screenshot .*phone", "capture .*screen"),
    ...
    IntentRule(
        "app_icons",
        ("ايقونات التطبيق", "احجام الايقونات", "app icon", "icon size"),
        (ArgSpec(FILE, "pick_image", filetypes="image"),
         ArgSpec(DIR, "pick_out_dir")),
    ),
    IntentRule(
        "make_logo",
        ("اعملي لوجو", "صمم لوجو", "logo", "make .*logo"),
    ...
    IntentRule(
        ...
        ("فحص البيئه", "الادوات المتثبته", "ايه الناقص", "ايه المتثبت",
         "environment", "env check", "what.*installed", "what.*missing",
         "check .*setup"),
    ),
    ...
    IntentRule("brain_status", ("حاله المخ", "المخ شغال", "الحصه",
     "brain status", "quota", "which model", "how many.*left")),
)
