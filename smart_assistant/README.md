...
شكل طرفية (terminal): عمود واحد بيمشي من فوق لتحت، خط مونوسبيس، وضع
داكن افتراضي. مفيش سايدبار ولا كروت — المحادثة نفسها هي الواجهة.

```
✻ nezuko
  مساعدتك الذكية المصرية

› افحص الملف ده أمنيًا
⏺ security_report C:\Users\me\file.exe
  ⎿ 0 تهديد · 12 ملف اتفحص · نضيف

╭──────────────────────────────────────────╮
│ › اكتب أي حاجة...                        │
╰──────────────────────────────────────────╯
 ⏵ gemini · 1,500 متبقي · 34 إضافة
```

| العلامة | معناها |
|---|---|
| `›` | سطر كتبته إنت |
| `⏺` | رد نيزوكو، أو أمر اتنفذ |
| `⎿` | نتيجة الأمر اللي فوقه |
| `⏵` | سطر الحالة (المخ النشط + الحصة) |

**أوامر الواجهة** (بتبدأ بـ `/`، ومبتلمسش المحرك ولا الحصة):

```
/settings   قايمة الإعدادات (أو دوس ⚙)
/clear      محادثة جديدة
/theme      داكن ⇄ فاتح
/voice      تشغيل/إيقاف الصوت
/lang       عربي ⇄ English
/quit       خروج
```
...
قايمة بصفحات بتتنقل فيها بالكيبورد: `↑ ↓` تنقل، `Enter` تغيير،
`←` رجوع، `Esc` خروج. كل أنظمة نيزوكو في مكان واحد:

| الصفحة | بتعمل إيه |
|---|---|
| **المخ** | مفاتيح المزوّدين المجانيين، الحصة، ومين شغال |
| **الموصلات (MCP)** | خوادم MCP — اتصال/فصل، وعدد أدوات كل واحد |
| **الإضافات** | المحمّلة، والمولّدة المستنية موافقتك |
| **المهارات** | أكتر الأوامر استخدامًا + تغطية القاموس المحلي |
| **الماكروهات** | أوامرك المخصصة من `commands/*.txt` |
| **الجلسات** | محادثاتك المحفوظة — افتح أي واحدة وكمّل عليها |
| **الصلاحيات** | الأوامر اللي بتعدي من غير سؤال |
| **الأحداث** | أوامر بتتنفذ لوحدها عند أحداث معيّنة |
| **الواجهة** | الثيم، اللغة، الصوت |

### الجلسات — محادثاتك مبتضيعش

المحادثة بتتحفظ على القرص بعد كل دور، فقفل البرنامج مبيضيّعش الكلام.
العنوان بيتولّد من أول رسالة **محليًا** — من غير أي نداء نموذج.

```
sessions                 كل المحادثات المحفوظة
session_open <id>        افتح واحدة وكمّل عليها
session_delete <id>      امسح واحدة (أو --all)
```
من الواجهة: `/resume` أو صفحة الجلسات في الإعدادات.

كل جلسة ملف JSON لوحده في `sessions/` (محلي، خارج git). الكتابة ذرّية،
فلو البرنامج اتقفل فجأة وسط الحفظ، الملف القديم يفضل سليم بدل ما
يتقطع. أقدم من 100 جلسة بتتشال تلقائيًا.

### الصلاحيات — لما السؤال في كل مرة يبقى ضوضاء

الافتراضي إن **مفيش أي حاجة بتتنفذ لوحدها**. لو بتعمل نفس النوع من
المهام كتير، تقدر تسمح لأمر معيّن يعدي:

```
allow <command>       خليه يعدي من غير سؤال
allow_list            شوف المسموح والممنوع
disallow <command>    اسحب السماح (أو --all)
```

وأسهل طريقة: لما نيزوكو تسألك تأكيد، اكتب **`a`** بدل `y` — هينفذ
ويتضاف للقايمة في نفس الوقت.

**أوامر ممنوعة نهائيًا مهما حصل** (`run`, `create_plugin`, `fix_plugin`,
`approve_plugin`, `android_shell`, `db_query`, `external_add`,
`quarantine_restore`): دي بتنفذ كود أو بتوصل لحاجة بمدخلات حرة، فمحتاجة
عين بشرية في كل مرة. المنع في الكود نفسه — حتى لو عدّلت ملف الإعدادات
بإيدك.

### الأحداث (hooks) — أتمتة عند حدث مش على معاد

```
hook add after_command echo خلص {command}
hook list
hook remove <event> <command>
hook clear
```

الأحداث: `startup`, `before_command`, `after_command`, `on_error`,
`session_end`. و`{command}` بتتبدل باسم الأمر اللي شغّل الحدث.

الأوامر المربوطة بتمشي في **نفس الطابور العادي**، يعني الأوامر الخطيرة
لسه بتتسأل — مفيش مسار مختصر. وتنفيذ أمر جاي من hook مبيولّدش أحداث
جديدة، وإلا `after_command` كان هيشغّل نفسه بلا نهاية.

### ظبّط "المخ" أول حاجة (دقيقتين، مجاني بالكامل)

نيزوكو بتفهم كلامك العادي بالعامية — مش لازم تحفظ أوامر. عشان كده
محتاجة نموذج لغة. **كل الخيارات دي مجانية دايمة ومن غير كارت ائتمان**،
والأسرع منهم Gemini:

1. افتح https://aistudio.google.com/apikey واعمل مفتاح بحساب جوجل
2. من نيزوكو، دوس 🧠 في الجنب (أو اكتب `brain_setup`) والصق المفتاح

بدايل مجانية تانية تقدر تضيفها كمان — **والحدود بتتجمع**، والانتقال
بينهم بيوزّع الطلبات فمتوصلش لحد الدقيقة بتاع أي واحد:

| المزوّد | الحد المجاني | التسجيل |
|---|---|---|
| Google Gemini | ~1,500 طلب/يوم، سياق مليون توكن | https://aistudio.google.com/apikey |
| Groq | ~1,000 طلب/يوم، أسرع استجابة | https://console.groq.com/keys |
| Cerebras | ~900 طلب/يوم | https://cloud.cerebras.ai |
| OpenRouter | ~200 طلب/يوم، عشرات النماذج | https://openrouter.ai/keys |
| Mistral | ~500 طلب/يوم | https://console.mistral.ai/api-keys |
| **Ollama (محلي)** | بلا حدود، أوفلاين بالكامل | https://ollama.com |

**الخصوصية:** الخطة المجانية لـ Gemini وMistral بتقول صراحةً إن كلامك
ممكن يستخدموه لتحسين نماذجهم. كل رد بيعرض المزوّد اللي ردّ، وفيه وضع
"محلي بس" (`brain_local on`) بيقفل كل السحابي ويشغّل Ollama بس.

### الحصة بتتصرف إمتى بالظبط؟

**الأدوات نفسها مجانية تمامًا** — اللي بيستهلك حصة هو المخ اللي بيقرر
أنهي أداة يستخدم. عشان كده معظم شغلك اليومي بيمشي بصفر حصة:

| اللي تعمله | التكلفة |
|---|---|
| أمر مضبوط (`virus_scan file.exe`) | 🆓 صفر |
| غلطة إملائية (`virus_scn`) | 🆓 صفر — `difflib` بيصححها محليًا |
| جملة في القاموس ("افحص الملف ده") | 🆓 صفر — `intents.py` |
| صيغة اتعلمتها قبل كده | 🆓 صفر — بتتحفظ للأبد أول مرة |
| أزرار الواجهة (📎 🛡️ 🎤) | 🆓 صفر |
| كلام حر جديد | ⚡ نداء واحد |

~96% من الأوامر المسجّلة بتتحل محليًا. وكل صيغة كلام المخ يفهمها
بتتحفظ في `intent_cache.json`، فنفس الصيغة تاني مرة بتبقى مجانية —
يعني كل نداء بتدفعه **مرة واحدة بس في حياتك**. شوف `brain_dict`
للتغطية الحالية.
...
- `main_gui.py` — واجهة المستخدم (CustomTkinter)، فيها زرار 📎 لرفع
  ...
  للملفات الأصغر بس، حفاظاً على الأداء. الأزرار اللي أيقونتها بس من
  غير نص واضح (➤ الإرسال، ▶/⏹ تشغيل/إيقاف المحرك) عندها تلميح (tooltip)
  عبر [CTkToolTip](https://github.com/Akascape/CTkToolTip) بيوضّح
  الأمر الحقيقي اللي بيتنفذ (زي `probe`/`security_report`) — مكتبة
  اختيارية خفيفة جدًا (مفيش لها أي dependencies)، لو مش متثبتة الواجهة
  تشتغل عادي من غير تلميحات.
...
- `plugins/` — كل إضافة ملف `.py` مستقل بيسجل أوامره بنفسه:
  ...
  - `voice_plugin.py` — صوت نيزوكو (`speak`, `voice_status`؛ شرح تفصيلي
    ...
    `listen_run`, `stt_status`؛ شرح تفصيلي تحت): تسجيل من المايك بيقف
    لوحده لما تسكت (silero-vad) + تفريغ لنص عبر سلسلة احتياطية من
    faster-whisper → whisper CLI → Vosk.
...
## أربع فجوات كانت بتخلي نيزوكو أضعف — واتقفلت

المقارنة مع Claude طلّعت أربع نقط نيزوكو كانت خسرانة فيها. تلاتة منهم
مكانوش مسألة "الموديل ضعيف" — كانوا **نظام ناقص**، وده اللي خلاهم
يتقفلوا من غير ما نغيّر مزوّد ولا ندفع حاجة.

### 1. بتسأل بدل ما تخمّن — بروتوكول `ASK:`

الطلب الغامض ("افحص الملف" وإنت فاتح تلات ملفات) كان بيتنفذ على أقرب
تخمين. دلوقتي النموذج يقدر يسأل سؤال توضيحي:

```
› افحص الملف
⏺ ❓ أنهي ملف بالظبط؟ فيه تلاتة مفتوحين عندك.
› التقرير
⏺ 🔧 محتاجة أشغّل: probe report.pdf
```

نقطة تنفيذ مهمة: إجابتك على السؤال **بتروح للمخ مباشرة** — مش
بتعدي على القاموس المحلي الأول. لو عدّت، رد زي "التقرير" كان
هيتفهم كأمر مش كإجابة.

وقاعدة أمان: لو النموذج خالف التعليمات وبعت `ASK:` و `TOOL:` مع بعض،
**السؤال بيكسب**. تنفيذ أمر مبني على تخمين النموذج نفسه شكّك فيه هو
أسوأ احتمال ممكن.

### 2. ذاكرة بتتلخّص بدل ما تتقص — Compaction

قبل كده: `del chat_history[:-30]` — الدور الواحد وتلاتين بيختفي خالص.
بعد نص ساعة شغل، نيزوكو مش فاكرة إنك أصلاً قلت إنك شغال على مشروع
معيّن.

دلوقتي أقدم جزء بيتحوّل لملاحظات مضغوطة بتفضل مثبتة في السياق:
المشروع اللي شغال عليه، القرارات اللي اتاخدت، مسارات الملفات، القيم
اللي إديتها، واللي لسه معلّق. **نفس نظام الـ compaction اللي Claude
Code شغال بيه.**

بيكلّف نداء نموذج واحد كل ~18 رسالة — مش كل رسالة. والملخص بيتراكم
مش بيتستبدل، فأقدم حاجة متضيعش في الضغطة اللي بعدها. ولو التلخيص فشل
(مفيش نت)، بيرجع للقص العادي — مش أسوأ من الأول.

### 3. بتفكر خطوة بخطوة في الأسئلة الصعبة

مؤشرات في اللغتين (`ليه`, `قارن`, `احسب`, `why`, `compare`, `debug`,
وأنماط حسابية) بتكشف السؤال اللي محتاج شغل مش استرجاع، وبتحقن تعليمة
تفكير متسلسل. **ده مجاني تمامًا من ناحية الحصة** — بيزود توكنات الخرج
شوية بس مش بيزود عدد النداءات.

وفيه وضع تالت جديد: `brain_mode auto` — الوضع العميق (MoA) يشتغل على
الأسئلة الصعبة **بس**، مش على كل رسالة. مقفول افتراضيًا عن قصد: العميق
بيستهلك ~4 أضعاف الحصة، وقرار صرف الحصة قرارك مش قرار البرنامج.

```
brain_mode normal    # نموذج واحد، أسرع وأرخص
brain_mode auto      # عميق في الصعب بس
brain_mode deep      # عميق دايمًا
```

### 4. سلسلة خطوات — نتيجة الأمر بترجع للمخ

دي كانت أكبر فجوة في شغل الكود. النموذج كان بيقترح أمر واحد وخلاص،
ونتيجته **عمرها ما بترجعله**. يعني "شغّل الاختبارات → اقرا الخطأ →
صلّحه" مكانتش ممكنة أصلاً — وده بالظبط الشكل اللي بيخلي مساعد يعرف
يصلّح كود فعلاً.

دلوقتي خرج الأمر بيرجع للمخ ويقدر يقترح الخطوة اللي بعدها، لحد 4 خطوات
في الطلب الواحد. السقف موجود عشان حلقة غلط متولّعش الحصة.

**كل خطوة في السلسلة لسه بتعدي على نفس التأكيد** — السلسلة بتخلي
نيزوكو تعرف تخطط، مش تعرف تنفذ من غير إذنك.
...
من أربع محركات، من الأفضل جودة للأضمن توفر — كل واحدة بيتجرب لو اللي
...
1. **edge-tts** — مجاني بالكامل من غير API key أو اشتراك، وفيه صوت
   ...
   مش نموذج محلي 100%.
2. **sherpa-onnx** — المحرك المحلي الأساسي دلوقتي (Apache-2.0، من
   k2-fsa). بيشغّل نفس نماذج Piper العصبية، بيتحمّل صوت اللغة
   (~65MB) مرة واحدة وبعدين offline بالكامل. شغّال كمكتبة بايثون
   مباشرة مش subprocess، فأسرع وأنضف. **السبب الحقيقي لوجوده تحت.**
3. **Piper (البرنامج)** — احتياطي للي عنده التثبيت القديم شغال. مش
   مطلوب لتثبيت جديد: sherpa فوق بيشغّل نفس الأصوات بالظبط.
...
4. **espeak-ng** — مضمّن محليًا دايمًا (لازم تتثبته يدوي على الجهاز،
...
### ليه sherpa-onnx دخل قبل Piper

مستودع **Piper الأصلي** ([rhasspy/piper](https://github.com/rhasspy/piper))
— اللي رخصته MIT — **اتأرشف read-only في أكتوبر 2025**، والتطوير انتقل
لـ [OHF-Voice/piper1-gpl](https://github.com/OHF-Voice/piper1-gpl)
بترخيص **GPL-3.0**. يعني كنا واقفين على مكتبة مش هتتصلح تاني، والبديل
الرسمي رخصته أتقل بكتير على برنامج بيتوزّع كـ exe.

sherpa-onnx بيحل المشكلتين مرة واحدة: محرك **مصان** برخصة **Apache-2.0**
بيشغّل **نفس نماذج Piper الأصلية بترخيص MIT**. مش استبدال للأصوات —
استبدال للمحرك اللي بيشغّلها.

الحزم اللي بتتحمّل هي نفس نماذج Piper معادة التغليف مع `tokens.txt`
وبيانات espeak-ng اللي المحرك محتاجها. فك الأرشيف بيعدي على فحص مسارات
صريح (`_safe_extract`) — أرشيف متحمّل من الإنترنت ممكن يكون فيه `../..`
أو رابط رمزي بيطلع بره المجلد، والفك من غير فحص ثغرة معروفة مش احتمال
نظري.

### وKokoro-82M؟

جودته أعلى فعلاً للإنجليزي ورخصته Apache-2.0 — بس **مفيهوش عربي خالص**
(8 لغات، مش منهم العربي)، وحزمته **304MB** مقابل 65MB. فمينفعش يكون
المحرك الأساسي لمساعد ثنائي اللغة. sherpa-onnx بيدعمه أصلاً، فلو
احتجته للإنجليزي بس هو إضافة صغيرة على نفس المسار.

و**fish-speech** مرفوض: النماذج بترخيص CC-BY-NC (غير تجاري) — نفس سبب
رفض openWakeWord، ومش متماشي مع مبادئ المشروع.
...
```
listen              # سجل لحد ما تسكت وفرّغ الكلام لنص، من غير تنفيذ
listen 10            # سجل 10 ثواني بالظبط بدل ما تستنى السكوت (أقصى حد: 30)
listen_run            # زي listen، لكن بعد التفريغ بينفذ النص كأمر فورًا
stt_status            # حالة sounddevice + المحركات التلاتة + VAD (متثبتين؟)
```

### وقف التسجيل لما تسكت (silero-vad)

`listen` من غير وسائط مكانش بيستنى إنك تخلص كلام — كان بيسجّل **مدة
ثابتة** (5 ثواني) مهما حصل. يعني لو رديت في ثانيتين بتستنى التلاتة
الباقيين بالعافية، ولو محتاج تمن ثواني بيتقطع عليك في نص الجملة.

الحل: **silero-vad** — نموذج كشف نشاط صوتي حجمه ~2 ميجا برخصة MIT،
بياخد أقل من 1ms للشريحة على معالج عادي، ومدرّب على أكتر من 6000 لغة
فبيشتغل على العربي والإنجليزي من غير أي تظبيط. دلوقتي `listen` من غير
وسائط بتسجّل لحد ما تسكت 800 مللي ثانية وبعدين تقف لوحدها.

تفاصيل التنفيذ اللي بتفرق عمليًا:

- **بنحتفظ باللي قبل أول كلمة** (300ms pre-roll). أول حرف في الجملة
  بيتقال قبل ما الـ VAD يقرر إن ده كلام، فمن غير ده التفريغ بيضيّع
  بداية الكلمة.
- **الضوضاء القصيرة مش كلام** (أقل من 300ms كلام = بنرفض التسجيل بدل
  ما نبعت صوت شبه فاضي للتفريغ). خبطة على الترابيزة مش جملة.
- **سقف أقصى 30 ثانية** برضه، عشان مايك سايب مفتوح مايفضلش يسجّل
  للأبد.
- **لو قلت `listen 10` بالظبط**، يبقى إنت عايز 10 ثواني — بنتجاهل
  الـ VAD تمامًا. الوسيط الصريح دايمًا بيكسب.

اختياري بالكامل: `pip install silero-vad`. من غيره كل حاجة تفضل شغالة
بالظبط زي الأول بالمدة الثابتة — `stt_status` بيوريك هو متثبت ولا لأ.
مش موجود في `Nezuko.exe` لأنه محتاج PyTorch (نفس سبب استبعاد
pyannote/Coqui — مشروح في `build_exe.bat`).
