...
696 اختبار حقيقي (مش placeholders) بتغطي كل plugin و Core Engine —
...
`sounddevice` (مكتبة بايثون خفيفة بـ PortAudio مبني جواها، من غير
تعقيد تحديد اسم جهاز صوت يدويًا)، والتفريغ لنص عبر `whisper` CLI
(حزمة `openai-whisper` مفتوحة المصدر بالكامل — أول استخدام بيحمّل
نموذجه، وبعدين offline تمامًا):
...
stt_status            # حالة sounddevice + whisper (متثبتين؟)
```
