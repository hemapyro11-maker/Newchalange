...
608 اختبار حقيقي (مش placeholders) بتغطي كل plugin و Core Engine —
...
النموذج اللي مثبّته على جهازك. لكن الفرق الحقيقي عن أي "شات بوت" بسيط
إن المساعد ده **بيقدر يستخدم أي أمر حقيقي من الـ 115+ أمر في نيزوكو
كأداة أثناء تفكيره** — مش بيخمّن، بيروح يجيب معلومة حقيقية:
...
- المحادثة بتفتكر اللي فات (متعددة الأدوار) طول الجلسة، لكنها مش
  متسجّلة على القرص — بتتصفر لما تقفل نيزوكو، أو دلوقتي بـ `think_reset`.

```
think <رسالتك>          # بداية/استمرار محادثة تفكير
think y / think n        # تأكيد أو رفض تشغيل أداة مقترحة
think_reset              # بداية جلسة جديدة
think_status             # حالة الجلسة الحالية (النموذج، عدد الرسائل)
